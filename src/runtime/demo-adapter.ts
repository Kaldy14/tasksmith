...
export class DemoRuntime implements RuntimeHandle {
  ...
  private async maybeWriteCiFixDemoChange(): Promise<void> {
    ...
    await this.sink.emit({ type: "command", command: "write TASKSMITH_DEMO_CI_FIXED.txt", toolCallId: "demo-write-ci-fix" });
  ...
  private async maybeWriteSecretDemoChange(): Promise<void> {
    ...
    await this.sink.emit({ type: "command", command: "write src/leaked-secret.ts", toolCallId: "demo-write-secret-change" });
    await this.sink.emit({ type: "command_output", command: "write src/leaked-secret.ts", output: "src/leaked-secret.ts\n", toolCallId: "demo-write-secret-change" });
  }
